...
//
// `wait_for!` evaluates to an Instance (and its specific class type) at compile time 
// based on live information from Roblox Studio.
//
// Here, we still have to specify what kind of message the RemoteEvent handles, so we 
// must tell the compiler explicitly.
...
fn RoundHandler() {
    ...
    // Services like `Players` are available as singleton instances.
